...
WHERE D.DELIVERERID IN 
                    (SELECT P1.DELIVERERID
                     FROM PENALTIES P1, PENALTIES P2
                     WHERE P1.DATA = P2.DATA AND P1.DELIVERERID = P2.DELIVERERID 
...
SELECT C.DELIVERERID 
FROM COMPANYDEL C
WHERE C.DELIVERERID <> 57 AND C.COMPANYID = ( SELECT C.COMPANYID
                                              FROM COMPANYDEL C
                                              WHERE C.DELIVERERID = 57 );
                                              
/*  6  */

SELECT D1.DELIVERERID, D1.NAME
FROM DELIVERERS D1, PENALTIES P1
WHERE D1.DELIVERERID = P1.DELIVERERID AND
      P1.DATA >= TO_DATE('01/01/1980','DD/MM/YYYY') AND
      P1.DATA <= TO_DATE('31/12/1980','DD/MM/YYYY')
GROUP BY D1.DELIVERERID, D1.NAME
HAVING COUNT(P1.PAYMENTID) > ( SELECT COUNT(P2.PAYMENTID)
                               FROM PENALTIES P2
                               WHERE P2.DELIVERERID = D1.DELIVERERID AND
                               P2.DATA >= TO_DATE('01/01/1981','DD/MM/YYYY') AND
                               P2.DATA <= TO_DATE('31/12/1981','DD/MM/YYYY') )
ORDER BY D1.DELIVERERID;
...
SELECT C.DELIVERERID 
FROM COMPANYDEL C
WHERE C.DELIVERERID <> 57 AND C.COMPANYID IN ( SELECT C.COMPANYID
                                              FROM COMPANYDEL C
                                              WHERE C.DELIVERERID = 57 )
GROUP BY C.DELIVERERID 
HAVING COUNT (*) = ( SELECT COUNT (*)
...
SELECT C.DELIVERERID 
FROM COMPANYDEL C
WHERE C.DELIVERERID <> 57 AND C.DELIVERERID NOT IN
                                    ( SELECT DELIVERERID 
                                      FROM COMPANYDEL C 
